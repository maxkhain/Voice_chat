...
def save_cache(target_ip, microphone_device_id=None, speaker_device_id=None):
    """
    ...
    Args:
        ...
        speaker_device_id (int, optional): Device ID of selected speaker
    """
    try:
        cache = {
...
def get_last_speaker():
    """
    Get the last used speaker device ID.
    
    Returns:
        int: Last speaker device ID, or None if not cached
    """
    cache = load_cache()
    return cache.get('speaker_device_id')
...
def display_cache_info():
    ...
    last_speaker = cache.get('speaker_device_id')
    ...
    if last_speaker is not None:
        print(f"  Last Speaker ID: {last_speaker}")
    else:
        print(f"  Last Speaker ID: (none)")
    print()
